<template lang="">
    <nuxt-link :to="`${BASE}${link}/${item?.id}/${item?.preview_url}`" class="item-container flex flex-col gap-2 rounded-md hover:bg-gray-950 bg-gray-900 p-3 pb-1 w-[13rem] h-[17rem] cursor-pointer max-sm:hidden playlist-container--desktop">
                    <!-- Item poster -->
    ...
    <!-- Mobile -->
                <nuxt-link :to="`${BASE}${link}/${item?.track?.album?.id}/${item?.track?.preview_url}`" v-for="item in items" class="item-container lg:hidden rounded-md hover:bg-gray-950 bg-gray-900 p-0 w-[13rem] h-[14rem] overflow-hidden playlist-container--mobile">
                    <!-- Item poster -->
